...
async function runAPITests() {
  ...
  try {
    ...
    if (!cadastroResponse.body.success) {
      console.log(`   ⚠️  Mensagem: ${cadastroResponse.body.message}`);
    }
    ...
    if (userId && voosDisponiveisResponse.body.success && voosDisponiveisResponse.body.voos.length > 0) {
      ...
      console.log('7. ⏭️  Pulando teste de compra - sem usuário ou voos disponíveis');
    }
    ...
    if (pilotosResponse.body.success) {
      console.log(`   ✈️  Pilotos disponíveis: ${pilotosResponse.body.pilotos ? pilotosResponse.body.pilotos.length : 0}`);
    }
    ...
    if (comissariosResponse.body.success) {
      console.log(`   👨‍✈️ Comissários disponíveis: ${comissariosResponse.body.comissarios ? comissariosResponse.body.comissarios.length : 0}`);
    }
